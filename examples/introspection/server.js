...
// Public method with exposed schema
rpc.addMethod('multiply', async (req, ctx, params) => {
  return params.a * params.b;
}, {
  schema: {
    type: 'object',
...
rpc.addMethod('internalCalculation', async (req, ctx, params) => {
  return params.value * 2 + 42;
}, {
  schema: {
    type: 'object',
    properties: {
...
rpc.addMethod('echo', async (req, ctx, params) => {
  return params;
}, {
  exposeSchema: true,
...
app.listen(PORT, () => {
  console.log(`RPC Server with introspection running on http://localhost:${PORT}/api`);
